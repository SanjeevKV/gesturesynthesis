function createPitchMFCCAngleMatrix(Language,Subject)

    DIR='F:/IIScProjectMain/Optitrack/ExtractedData/';
    markerDIR = [DIR Language '/' Subject '/Marker/'];
    saveDIR=['F:/IIScProjectMain/Optitrack/Analysis/PitchMFCCAngles_70/' Language '/' Subject '/'];
    ...
    for i=1:length(allWavsEn)
        ...
        mfcc=readhtk(mfccfile);t_mfc=[1:size(mfcc,1)]/100;
...
%         measuresEn.mean=[];measuresEn.std=[];measuresEn.median=[];measuresEn.kurt=[];measuresEn.dur=[];
        ...
        for j=1:size(nonZeroTIntervals,1)
            ...
            mfcc_inds=t_mfc>=startT & t_mfc<=endT;
            
            m_data=angles(m_inds,:);
            p_data=pitch(p_inds);
            mfcc_data=mfcc(mfcc_inds,:);
