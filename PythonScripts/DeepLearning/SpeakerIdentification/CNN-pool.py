...
def functional_CNN_pool(input_shape, num_classes):
    ...
    conv_output = Conv1D(24, 1, activation = 'linear', name = "convolution")(motion_input)
...
train_generator_obj = SpeakerIdentificationBatchGenerator(os.path.join(DATA_ROOT_LOCATION, "train"), vfps = VFPS,  num_steps = NUM_STEPS, batch_size = BATCH_SIZE, skip_steps=NUM_SKIP_STEPS, out_size = OUT_SIZE, input_channels = NUM_CHANNELS)
valid_generator_obj = SpeakerIdentificationBatchGenerator(os.path.join(DATA_ROOT_LOCATION, "valid"), vfps = VFPS, num_steps = NUM_STEPS, batch_size = BATCH_SIZE, skip_steps=NUM_SKIP_STEPS, out_size = OUT_SIZE, input_channels = NUM_CHANNELS)

model = functional_CNN_pool(input_shape=(train_generator_obj.num_steps_motion, NUM_CHANNELS),num_classes=OUT_SIZE)
model.summary()
...
model.fit_generator(generator = train_generator_obj.generate_head_motion_data_for_speaker_identification_sin_rand(column_names = COLUMNS), epochs = NUM_EPOCHS, steps_per_epoch = train_steps_per_epoch,
                    validation_data = valid_generator_obj.generate_head_motion_data_for_speaker_identification_sin_rand(column_names = COLUMNS), validation_steps = valid_steps_per_epoch, callbacks = callbacks_list)
