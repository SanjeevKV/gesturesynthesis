...
class Dataset(object):
    def __init__(self, speaker_id, name, motion_data_folder, motion_files, num_steps_motion, skip_steps_motion):
        self.speaker_id = speaker_id
        self.name = name
        self.motion_data_folder = motion_data_folder
        self.motion_data_files = motion_files
...
class Speaker(object):
    def __init__(self, speaker_id, motion_data_folder, num_steps_motion, skip_steps_motion, split):
        self.speaker_id = speaker_id
        self.motion_data_folder = motion_data_folder
        ...
        np.random.shuffle(self.motion_data_files)
        #print(self.motion_data_files)
        #self.total_motion_files = len(self.motion_data_files)
        #self.current_file_idx = 0
        #self.current_file_offset = 0
        #self.motion_data = pd.DataFrame()
        ...
        self.datasets = {}
        self.assign_datasets() # Current Motion File's data - Assigned to self.motion_data
     
    def assign_datasets(self):
        start_idx = 0
        for i in range( len(self.split) ):
            cur_idx = start_idx + self.split[i]         
            self.datasets[ self.dataset_names[i] ] = Dataset(self.speaker_id, self.dataset_names[i], self.motion_data_folder, self.motion_data_files[start_idx : cur_idx], 
            self.num_steps_motion, self.skip_steps_motion)
            start_idx = cur_idx
...
class SpeakerIdentificationBatchGenerator(object):
    def __init__(self, motion_data_path, num_steps, batch_size = 10, vfps = 25, skip_steps=0.5, out_size = 2, input_channels = 1, train = True, split = [6,2,2]):
    ...
    def initialize_speakers(self):
        ...
        for id_num, mdf in enumerate(self.motion_data_folders):
            self.speakers.append(Speaker(id_num, os.path.join(self.motion_data_path,mdf), self.num_steps_motion, self.skip_steps_motion, self.split))
    ...
    """        
    def generate_head_motion_data_for_speaker_identification_mul_rand(self, column_names, layers_names, number_of_chunks, dataset_name):
        x = np.zeros((self.batch_size, self.num_steps_motion, self.input_channels))
        y = np.zeros((self.batch_size, self.out_size))
        while True:
            for i in range(self.batch_size):
                    
                x[i, :, :], y[i, :] = self.speakers[np.random.randint(self.out_size)].datasets[dataset_name].return_sample(self.num_steps_motion, 
    ...
    """        
    def generate_head_motion_data_for_speaker_identification_sin_rand(self, column_names, dataset_name):
        x = np.zeros((self.batch_size, self.num_steps_motion, self.input_channels))
        y = np.zeros((self.batch_size, self.out_size))
        while True:
            for i in range(self.batch_size):
    ...
    """        
    def generate_head_motion_data_for_speaker_identification_sin_seq(self, column_names, dataset_name):
        x = np.zeros((self.batch_size, self.num_steps_motion, self.input_channels))
        y = np.zeros((self.batch_size, self.out_size))
        while True:
            for i in range(self.batch_size):
                    
                x[i, :, :], y[i, :] = self.speakers[self.speaker_counter].datasets[dataset_name].return_sample(self.num_steps_motion, 
                                             self.skip_steps_motion, column_names, self.out_size)
