...
def cnn1d(input_shape, num_classes):
    ...
    # LFLB1
    model.add(Conv1D(filters = 2,kernel_size = (CONV_1D_KERNEL_SIZE),strides=1,padding='same',data_format='channels_last',input_shape=input_shape))	
    ...
    #LFLB2
    model.add(Conv1D(filters=2, kernel_size = CONV_1D_KERNEL_SIZE, strides=1,padding='same'))
    model.add(BatchNormalization())
    model.add(Activation('relu'))
    model.add(MaxPooling1D(pool_size = 4, strides = 2))
    #model.add(SpatialDropout1D(0.5))
...
def lstm_lstm(input_shape, num_classes):
    ...
    model.add(LSTM(150, return_sequences=True,activation='sigmoid'))  # returns a sequence of vectors of dimension 32
...
train_generator_obj = SpeakerIdentificationBatchGenerator('/home2/data/Sanjeev/data/CoordinatesData/train', num_steps = 30, batch_size = batch_size, skip_steps=10, out_size = out_size, input_channels = channels)
valid_generator_obj = SpeakerIdentificationBatchGenerator('/home2/data/Sanjeev/data/CoordinatesData/valid', num_steps = 30, batch_size = batch_size, skip_steps=10, out_size = out_size, input_channels = channels)

model = cnn1d(input_shape=(train_generator_obj.num_steps_motion, channels),num_classes=out_size)
model.summary()
#sys.exit()
filepath="/home2/data/Sanjeev/models/speaker-identification-2019-09-03/weights-improvement-{epoch:02d}-{categorical_accuracy:.2f}-{val_categorical_accuracy:.2f}.hdf5"
